...
from API_testing_tool.main_1 import render_api_testing_tool
from API_testing_tool.presets import get_presets
load_dotenv()
...
with st.expander("Expand", expanded=False):
    response = render_api_testing_tool()  # Your API testing logic must return a response

    with st.expander("🧪 Test Case Validator", expanded=True):
        expected_status = st.number_input("Expected Status Code", value=200)
        expected_body = st.text_area("Expected Response Body (JSON)", height=150)

        if st.button("Validate Response"):
            if response is None:
                st.warning("⚠️ No API response to validate.")
            else:
                try:
                    parsed_expected = json.loads(expected_body)
                    parsed_actual = response.json()

                    is_pass = (
                        response.status_code == expected_status
                        and parsed_actual == parsed_expected
                    )
